...
export default new Vuex.Store({
  ...
  actions: {
    getImages({ commit }) {
      axios.get(`https://api.unsplash.com/photos?page=1&per_page=12&client_id=${API_KEY}`)
      .then(response => {
    ...
    searchImages({ commit, state }) {
      ...
      axios.get(`https://api.unsplash.com/search/photos?client_id=${API_KEY}&query=${state.searchTerm}&page=1&per_page=12`)
      .then(response => {
